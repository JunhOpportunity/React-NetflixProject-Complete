...
import { getTvToday, getTvOnAir, getTvPopular, IGetTvResult } from "../api";
...
//   query GetDetails($detailMovieId: ID) {
//     details(detailID: $movieId) {
//       genres
...
// const DelayedQuery = (movieId: number) => {
//   const [getDetails, { loading, error, data: DetailData }] = useLazyQuery(
//     Get_Details
//   );
...
//   return fetch(
//     `${BASE_PATH}/movie/${movieId}?api_key=${API_KEY}&language=kor`
//   ).then((response) => response.json());
//   return (
//     <div>
//       {DetailData ? <p>{}</p> : <p>!Data Not Found!</p>}
//       <button onClick={() => getDetails({ variables: { movieId } })}>
//         Click me!
//       </button>
//     </div>
//   );
...
function Home() {
  // const [fetchSuggestions, { loading, data }] = useLazyQuery(Get_Details);
  ...
  const { data: pageOneData, isLoading: pageOneLoading } = useQuery<
    IGetTvResult
  >(["nowplayingOneMovie", "nowPlayingOne"], getTvToday);

  const { data: pageTwoData, isLoading: pageTwoLoading } = useQuery<
    IGetTvResult
  >(["nowplayingTwoMovie", "nowPlayingTwo"], getTvOnAir);

  const { data: pageThreeData, isLoading: pageThreeLoading } = useQuery<
    IGetTvResult
  >(["nowplayingThreeMovies", "nowPlayingThree"], getTvPopular);

  console.log("pageData", pageOneData);
  ...
  const increaseIndexOne = () => {
    setBackOne(false);
    console.log("Increase:", isBackOne);
  ...
  const decreaseIndexOne = () => {
    setBackOne(true);
    console.log("decrease:", isBackOne);
    const totalMovies = pageOneData.results.length - 1; // 맨 처음에 Banner로 하나 사용했으므로 1 빼줌
    const maxIndex = Math.floor(totalMovies / offset) - 1; // page가 0에서부터 시작이라서 1 빼줌
  ...
  const increaseIndexTwo = () => {
    setBackTwo(false);
    console.log("Increase:", isBackTwo);
  ...
  const decreaseIndexTwo = () => {
    setBackTwo(true);
    console.log("decrease:", isBackTwo);
    const totalMovies = pageTwoData.results.length - 1; // 맨 처음에 Banner로 하나 사용했으므로 1 빼줌
    const maxIndex = Math.floor(totalMovies / offset) - 1; // page가 0에서부터 시작이라서 1 빼줌
  ...
  const increaseIndexThree = () => {
    setBackThree(false);
    console.log("Increase:", isBackThree);
  ...
  const decreaseIndexThree = () => {
    setBackThree(true);
    console.log("decrease:", isBackThree);
    const totalMovies = pageThreeData.results.length - 1; // 맨 처음에 Banner로 하나 사용했으므로 1 빼줌
    const maxIndex = Math.floor(totalMovies / offset) - 1; // page가 0에서부터 시작이라서 1 빼줌
    ...
    <Wrapper>
      {pageOneLoading ? (
        ...
        <>
          ...
          >
            <Title>{pageOneData?.results[0].name}</Title>
            <Overview>{pageOneData?.results[0].overview}</Overview>
          </Banner>

          <SliderTitle>현재 방영중인 콘텐츠</SliderTitle>
          <Slider>
            <AnimatePresence
              custom={isBackOne}
              initial={false}
          ...
          <SliderTitle>가장 인기있는 콘텐츠</SliderTitle>
          <Slider>
            <AnimatePresence
              custom={isBackTwo}
              initial={false}
