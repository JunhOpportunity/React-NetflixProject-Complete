...
export async function getMoviesOneDetails() {
  ...
  //   data.results.map((a) =>
  //     fetch(
  //       `${BASE_PATH}/movie/${a.id}?api_key=${API_KEY}&language=kor`
  //     ).then((response) => response.json())
  //   )
  // );
}
...
export const detail = () => {
  ...
  const detailData = data.results.map((response) =>
    fetch(
      `${BASE_PATH}/movie/${response.id}?api_key=${API_KEY}&language=kor`
    ).then((response) => response.json())
  );
...
// export const fetchDetail = new Promise(() => {
//   fetch(`${BASE_PATH}/movie/now_playing?api_key=${API_KEY}&language=ko&page=1`)
//     .then((response) => response.json())
//     .then((data) =>
//       data.results.map((a) =>
//         fetch(
